...
var activityIndicator = new tabris.ActivityIndicator({
  ...
  centerY: 0
    ...
    window.plugins.socialsharing.share(newsItem.title,'Radio360 - '+newsItem.title,null,newsItem.link);
  });
...
function displayVideo(newsItem,page) 
{
  ...
  url: "http://www.myradio360.com/api/article/"+newsItem.id+"?"+new Date().getTime(),
  }).appendTo(page);
}
...
function fetch_newsDetails(newsItem,newsArticle,activityIndicator,contentComposite)
{
   ...
   utils.getJSON(json_url).then(function (json) {
       ...
       var data=json;
         ...
         {
					 var articleArticle=data.article;
					 if(data.article!='')
					 {
